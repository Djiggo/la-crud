...
$user_path = ("../../../../files/" . $default_upload_dir);
$user_thumbs_path = ("../../../../files/thumbs/" . $default_upload_dir);

if (!file_exists($user_path)) {
    if (!mkdir($user_path, 0777, true)) {
        throw new \Exception("Error create files directory");
    }
}
if (!file_exists($user_thumbs_path)) {
    if (!mkdir($user_thumbs_path, 0777, true)) {
        throw new \Exception("Error create thumb directory");
    }
}
...
//remember last position
    ...
    if (!is_dir($thumbs_path . $subdir)) {
        $f = 1;
        //create_folder(FALSE, $thumbs_path . $subdir); // Чтот я запутался в этом скрипте...
    }
...
// language
        ...
        <div class="uploader">
            ...
            <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
                <ul class="nav nav-tabs">
                    ...
                    <li><a href="#tab3" id="uploader-btn" data-toggle="tab">Скачать по ссылке</a></li>
                </ul>
                ...
                <div class="tab-pane" id="tab3">
                    <div id="upload_by_url-container" class="upload_by_url">
                        <form action="upload_by_url.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="control-group">
                                <div>
                                    <input type="text" class="file_url" name="files[0][url]" value=""
                                           placeholder="Ссылка на файл"/>
                                </div>
                                <div>
                                    <input type="text" class="file_name" name="files[0][name]" value=""
                                           placeholder="Название файла"/>
                                </div>
                            </div>
                            <div class="control-group">
                                <div>
                                    <input type="text" class="file_url" name="files[1][url]" value=""
                                           placeholder="Ссылка на файл"/>
                                </div>
                                <div>
                                    <input type="text" class="file_name" name="files[1]" value=""
                                           placeholder="Название файла"/>
                                </div>
                            </div>
                            <div class="control-group">
                                <div>
                                    <input type="text" class="file_url" name="files[2][url]" value=""
                                           placeholder="Ссылка на файл"/>
                                </div>
                                <div>
                                    <input type="text" class="file_name" name="files[2][name]" value=""
                                           placeholder="Название файла"/>
                                </div>
                            </div>
                            <div>
                            ...
                            <div class="upload-help">Можно загрузить от 1 до 3 картинок.</div>
                        </form>
                        <script>
                            jQuery(function ($) {

                                var by_url_form = $('.upload_by_url form');

                                by_url_form.submit(function (e) {
                                    e.preventDefault();
                                    $.post(by_url_form.attr('action'), by_url_form.serialize(), function (data) {
                                        if (data == "ok") {
                                            $(".upload_by_url input").val("");
                                            $(".close-uploader").click();
                                        }
                                    });
                                });

                                $(".upload_by_url .file_url").bind('input', upload_by_url_change);

                                function upload_by_url_change() {
                                    var form_ok = true;

                                    jQuery(".upload_by_url .file_url").each(function () {
                                        if (!$(this).val().length) {
                                            return;
                                        }
                                        var file_name = $(this).val().match(/([^\/]+)$/)[1];
                                        var file_name_div = $(this).closest(".control-group").find('.file_name');

                                        if (!file_name.match(/\.(jpg|gif|png)$/)) {
                                            form_ok = false;
                                            file_name_div.css('color', 'red');
                                            file_name_div.val("Не стоит это загружать!");
                                        }
                                        else {
                                            file_name_div.val(file_name);
                                            file_name_div.css('color', "");
                                        }
                                    });

                                    if (form_ok) {
                                        $(".upload_by_url button").prop('disabled', false);
                                    }
                                    else {
                                        $(".upload_by_url button").prop('disabled', true);
                                    }
                            ...
                            <div class="span6 half types">
                                ...
                                <?php if ($_GET['type'] != 1 && $_GET['type'] != 3) { ?>
                                    ...
                                    <input id="select-type-2" name="radio-sort" type="radio" data-item="ff-item-type-2"
                                           class="hide"/>
                                    <label id="ff-item-type-2" title="<?php echo lang_Images; ?>" for="select-type-2"
                                    ...
                                    <input id="select-type-3" name="radio-sort" type="radio" data-item="ff-item-type-3"
                                           class="hide"/>
                                    <label id="ff-item-type-3" title="<?php echo lang_Archives; ?>" for="select-type-3"
                                    ...
                                    <input id="select-type-4" name="radio-sort" type="radio" data-item="ff-item-type-4"
                                           class="hide"/>
                                    <label id="ff-item-type-4" title="<?php echo lang_Videos; ?>" for="select-type-4"
    ...
    foreach ($files as $file_array) {
        ...
        ?>
        <li data-name="<?php echo $file ?>" <?php
        if ($file == '..')
            echo 'class="back"';
        else
            echo 'class="dir"';
        ?>><?php
            ...
            ?>
            <figure data-name="<?php echo $file ?>" class="<?php if ($file == "..") echo "back-"; ?>directory"
                    data-type="<?php
                    if ($file != "..") {
                        echo "dir";
                    }
                ...
                <?php } else { ?>
                    ...
                    <figcaption>
                        <a href="javascript:void('')"
                           ...
                           title="<?php echo lang_Rename ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>"
            ...
            <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>"
               data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
                ...
                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                    <div
                        class="filetype <?php echo $extension_lower ?> <?php if (in_array($extension_lower, $editable_text_file_exts)) echo 'edit-text-file-allowed' ?> <?php
                        if (!$is_icon_thumb) {
                            echo "hide";
                        }
            ...
            <figcaption>
                <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                    ...
                    <?php if ($is_img && $src_thumb != "") { ?>
                        <a class="tip-right preview" title="<?php echo lang_Preview ?>" data-url="<?php echo $src; ?>"
                    ...
                    <?php } elseif (($is_video || $is_audio) && in_array($extension_lower, $jplayer_ext)) { ?>
                        ...
                        ?>"
                           ...
                           data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php
                    ...
                    <?php } elseif ($preview_text_files === TRUE && in_array($extension_lower, $previewable_text_file_exts)) { ?>
                        <a class="tip-right file-preview-btn" title="<?php echo lang_Preview ?>"
                           data-url="ajax_calls.php?action=get_file&sub_action=preview&title=<?php echo $filename; ?>&file=<?php
                           echo $current_path . $rfm_subfolder . $subdir . $file;;
                           ?>"
                           href="javascript:void('');"><i class=" icon-eye-open"></i></a>
                    <?php } else { ?>
