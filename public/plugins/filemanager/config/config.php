...
$default_upload_dir = 'users/' . $_SESSION['user_id'];


if (array_key_exists('HTTP_REFERER', $_SERVER) && strlen($_SERVER['HTTP_REFERER']) > 10) {
    $referal_upploaddir = [];
    preg_match('/&uploaddir=([^&]+)/', $_SERVER['HTTP_REFERER'], $referal_upploaddir);
    if ($referal_upploaddir) {
        $_GET['uploaddir'] = $referal_upploaddir[1]; // Костыль, чтоб работала загрузка файлов
    }
}

if (array_key_exists('uploaddir', $_GET)) {
    if (strpos("..", $_GET['uploaddir']) !== false) {
        echo "Hacking attempt";
        die();
    }

    $default_upload_dir = $_GET['uploaddir'];
}

$base_url = "http://" . $_SERVER['HTTP_HOST'];  // DON'T TOUCH (base url (only domain) of site (without final /)).
$upload_dir = '/files/' . $default_upload_dir . '/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../../../files/' . $default_upload_dir . '/'; // relative path from filemanager folder to upload folder (with final /)
//thumbs folder can't put inside upload folder
$thumbs_base_path = '../../../../files/thumbs/' . $default_upload_dir . '/'; // relative path from filemanager folder to thumbs folder (with final /)
// OPTIONAL SECURITY
